...
const normalizeItems = (items = []) =>
  items
    ...
    .map((it) => ({
      ...
      gst: Number(it.gst ?? 0),
      priceIncludesGst: Boolean(it.priceIncludesGst ?? true), // ✅ ADDED
...
// Helper: generate a unique key for merging
const itemKey = (it) => {
...
const toFrontendItem = (it) => ({
  ...
  gst: it.gst,
  priceIncludesGst: it.priceIncludesGst ?? true, // ✅ ADDED
  ...
  try {
    ...
    for (const it of incoming) {
      ...
      if (mergedMap.has(key)) {
        ...
        existing.gst = it.gst;
        existing.priceIncludesGst = it.priceIncludesGst; // ✅ ADDED
        existing.isCustomized = it.isCustomized;
        existing.image = it.image;
        existing.name = it.name;
        existing.attributes = it.attributes;
      } else {
        mergedMap.set(key, {
          ...it,
          _id: new mongoose.Types.ObjectId(),
